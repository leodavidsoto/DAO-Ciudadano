 ...
 * - Contains identity hash as bytes32 (not PII, not a mutable string)
 * - Role-separated: MINTER_ROLE, PAUSER_ROLE and REVOKER_ROLE are distinct
 *   from DEFAULT_ADMIN_ROLE (D-1 del ADR) — a compromised backend minter key
 *   only lets an attacker mint, not pause/revoke/re-grant roles.
 * - Pausable for emergencies
 * - Reentrancy protected
 * - Revocable by REVOKER_ROLE with cooldown
 */
contract DAOCiudadanaSBT is ERC721, ERC721URIStorage, AccessControl, Pausable, ReentrancyGuard {
    uint256 private _nextTokenId;

    // === Roles ===
    // DEFAULT_ADMIN_ROLE (from AccessControl) can grant/revoke every role
    // below. Keep it on a multisig/cold address, not the same key as
    // MINTER_ROLE (which the backend holds to mint autonomously).
    ...
    // Token ID to identity hash (bytes32 — matches app/core/identity.py's
    // HMAC-SHA256(pepper, rut) digest, not a free-form string)
     ...
     * PAUSER_ROLE and REVOKER_ROLE at deploy time. In production the admin
     * should grant MINTER_ROLE to the backend's hot wallet separately and
     * consider moving DEFAULT_ADMIN_ROLE to a multisig afterwards — deploying
     * with a single EOA holding every role is a starting point, not the
     * recommended steady state.
     */
    constructor(address admin) ERC721("DAO Ciudadana SBT", "DAOSBT") {
     ...
     * @param identityHash HMAC-SHA256(pepper, rut) of verified identity (not PII)
     ...
     */
    function requestRevocation(uint256 tokenId) public onlyRole(REVOKER_ROLE) {
     ...
     */
    function cancelRevocation(uint256 tokenId) public onlyRole(REVOKER_ROLE) {
     ...
     */
    function executeRevocation(uint256 tokenId, string memory reason) public onlyRole(REVOKER_ROLE) nonReentrant {
