...
public class PACEHandler {
    ...
    private static let CAN_PACE_KEY_REFERENCE : UInt8 = 0x02 // Wired locally; physical validation pending
    ...
    public func doPACE( mrzKey : String, paceKeyReference: UInt8 = 0x01 ) async throws {
        ...
        paceKeyType = paceKeyReference
        paceKey = try createPaceKey( from: mrzKey )
        
        // Never log the CAN-derived key or its input. OSLog is not an
        // acceptable boundary for document access credentials.
...
*/
    ...
    /// Performs PACE Step 1 using the selected key reference (MRZ or locally wired CAN).
...
///         - Tuple of Generated Ephemeral KeyPair and the Passport's public key
    ...
    func doStep4KeyAgreement( pcdKeyPair: OpaquePointer, passportPublicKey: OpaquePointer) async throws -> ([UInt8], [UInt8]) {
        ...
        guard piccToken == expectedPICCToken else {
            Logger.pace.error("PACE authentication token mismatch")
