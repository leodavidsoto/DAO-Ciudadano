...
public class TagReader {
    ...
    func selectFileAndRead( tag: [UInt8]) async throws -> [UInt8] {
        ...
        while remaining > 0 {
            ...
            Logger.tagReader.debug("TagReader - received \(resp.data.count) data bytes")
            data += resp.data
    ...
    func send( cmd: NFCISO7816APDU, useExtendedMode : Bool = false ) async throws -> ResponseAPDU {
        Logger.tagReader.debug("TagReader - sending APDU")
        var toSend = cmd
        ...
        if let sm = self.secureMessaging {
            ...
            Logger.tagReader.debug("TagReader - secure response unwrapped")
        } else {
            Logger.tagReader.debug("TagReader - unprotected response received")
        }
