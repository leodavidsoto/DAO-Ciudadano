...
public class PassportReader : NSObject {
    ...
    private var allowBACFallback = true
    private var paceKeyReference: UInt8 = 0x01
    ...
    public func readPassport( mrzKey : String, tags : [DataGroupId] = [], aaChallenge: [UInt8]? = nil, skipSecureElements : Bool = true, skipCA : Bool = false, skipPACE : Bool = false, useExtendedMode : Bool = false, paceKeyReference : UInt8 = 0x01, allowBACFallback : Bool = true, customDisplayMessage : ((NFCViewDisplayMessage) -> String?)? = nil) async throws -> NFCPassportModel {
        ...
        self.allowBACFallback = allowBACFallback
...
extension PassportReader : NFCTagReaderSessionDelegate {
    ...
    public func tagReaderSession(_ session: NFCTagReaderSession, didDetect tags: [NFCTag]) {
        Logger.passportReader.debug( "tagReaderSession:didDetect - found \(tags.count) tag(s)" )
        if tags.count > 1 {
            Logger.passportReader.debug("tagReaderSession:more than 1 tag detected")
...
extension PassportReader {
    ...
    func startReading(tagReader : TagReader) async throws -> NFCPassportModel {
        ...
        if passport.PACEStatus != .success {
            guard allowBACFallback else {
                throw NFCPassportReaderError.PACEError(
                    "PACE",
                    "PACE failed and BAC fallback is disabled"
                )
            }
            do {
    ...
    func doActiveAuthenticationIfNeccessary( tagReader : TagReader) async throws {
        ...
        Logger.passportReader.debug("Generated Active Authentication challenge")
    ...
    /// Cancel the CoreNFC session owned by this reader and settle any pending
    /// async read exactly once. The React Native bridge uses this instead of
    /// attempting to cancel a separate `react-native-nfc-manager` session.
    public func cancelPassportRead() {
        guard readerSession != nil || nfcContinuation != nil else { return }
